...
def eval():
    ...
    with torch.no_grad():
        for iteration, (ids, image_ids, imgs, input_ids, lm_labels, medterm_labels) in enumerate(tqdm_bar):
            ...
            print('output shape', output_global.shape, fm_global.shape, pool_global.shape)
            print('img shape', len(imgs), len(imgs[0]), len(imgs[0][0]), len(imgs[0][0][0]))
            ...
            print('output_fusion', output_fusion.size(), pool_global.size(), pool_local.size())
