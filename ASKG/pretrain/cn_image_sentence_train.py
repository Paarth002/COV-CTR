...
def Attention_gen_patchs(ori_image, fm_cuda):
    ...
    for i in range(0, bz):
        ...
        maxh = max(ind[:, 0])
        maxw = max(ind[:, 1])
        # minh, minw, maxh, maxw = 0, 0, 1, 1
        # to ori image
        # print(type(ori_image[i]))
        if i == len(ori_image):
            break
        image = ori_image[i].cpu().numpy().reshape(224, 224, 3)
        image = image[
            int(224 * 0.334) : int(224 * 0.667), int(224 * 0.334) : int(224 * 0.667), :
        ]

        image = cv2.resize(image, size_upsample)
        image_crop = (
            image[minh:maxh, minw:maxw, :] * 256
        )  # because image was normalized before
        image_crop = preprocess(
            Image.fromarray(image_crop.astype("uint8")).convert("RGB")
        )

        img_variable = torch.autograd.Variable(
            image_crop.reshape(3, 224, 224).unsqueeze(0).cuda()
        )
...
def abstracts_train():
    ...
    for iteration, (ids, abstracts, abstracts_labels, medterm_labels) in enumerate(
        tqdm_bar
    ):
        ...
        med_probs, abstracts_outputs = model(
            att_feats=abstracts, input_ids=abstracts, input_type="sentence"
        )
        med_loss = med_crit(med_probs, medterm_labels)

        caption_loss = outputs_crit(
            abstracts_outputs.view(-1, abstracts_outputs.size(-1)),
        ...
        tqdm_bar.desc = (
            "train_loss = {:.5f}, med_loss = {:.5f}, caption_loss = {:.5f},"
            "lr = {:.5f}, cnn_lr = {:.5f}".format(
                train_loss,
                med_loss,
                caption_loss,
                rnn_NoamOpt.optimizer.param_groups[0]["lr"],
                opt.cnn_learning_rate,
            )
        )
...
def images_train():
    ...
    for iteration, (
        ...
        med_porbs, findings_outputs = model(
...
def update_encoder_model(path):
    ...
    rnn_state_dict = {
        k: v for k, v in rnn_state_dict.items() if k in model_state_dict.keys()
    }
        ...
        if epoch % opt.val_every_epoch == 0:
            ...
            if best_flag:
                ...
                cnn_checkpoint_path = os.path.join(
                    opt.checkpoint_path, "model-cnn-best.pth"
                )
                torch.save(cnn_model.state_dict(), cnn_checkpoint_path)
                print("cnn model saved to {}".format(cnn_checkpoint_path))
...
def eval():
    ...
    with torch.no_grad():
        for iteration, (
            ...
            imgs2 = [[img.to(device) for img in sample] for sample in imgs]
            imgs = imgs.to(device)
            medterm_labels = medterm_labels.to(device)

            output_global, fm_global, pool_global = cnn_model(imgs)

            print("type", type(output_global), type(fm_global), type(pool_global))
            print(
                "output shape", output_global.shape, fm_global.shape, pool_global.shape
            )
            print(
                "img shape",
                len(imgs),
            ...
            # print('len', len(output_global), len(output_global[0]), len(output_global[0][0]), len(output_global[0][0][0]))
            patchs_var = Attention_gen_patchs(imgs, fm_global)
            ...
            med_porbs, findings_seq = model(
                att_feats=output_fusion, mode="inference", input_type="img"
            )
            # if len(findings_seq) > 512:
            #    findings_seq = findings_seq[:511]
            findings_samples = decode_transformer_findings(
                img_train_dataset.idw2word, findings_seq
            )
            findings_truths = decode_transformer_findings(
                img_train_dataset.idw2word, lm_labels
            )
    ...
    if opt.language_eval == 1:
        lang_stats = eval_utils.evaluate(
            id2captions_t,
            id2captions_g,
            save_to="./results/",
...
def test():
    ...
    print(
        "The average AUROC is {AUROC_avg:.3f}".format(AUROC_avg=AUROC_avg), len(AUROCs)
    )
...
if __name__ == "__main__":
    ...
    fusion_model = Fusion_Branch(input_size=1024, output_size=50176).cuda()  ###
    print("fusion model output options", opt.num_medterm)
    ...
    cnn_optimizer = optim.Adam(
        model.parameters(), lr=1e-4, weight_decay=opt.weight_decay
    )
    # load_best_model()
